...
export default function WelcomeText(props) {
  ...
  console.log(theme);
  useEffect(async () => {}, [controlsText, controlsButton]);

  const artistText = configurations[1].lines.map((line, i) => {
    if (i === 0) {
      return (
        <Grid
          initial="initial"
          animate="animate"
          variants={
            configurations[props.configurationNumber].topSentenceAnimationData
          }
  ...
  const buyerText = configurations[2].lines.map((line, i) => {
    if (i === 0) {
      return (
        <Grid
          initial="initial"
